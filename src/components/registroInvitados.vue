        ...
        <v-row class="pr-3 justify-end">
          <v-dialog v-model="dialog" persistent max-width="600px">
            <template v-slot:activator="{ on }">
              <v-btn color="#4db6ac" class="text-capitalize" dark v-on="on"> <v-icon >add_circle_outline</v-icon> Nuevo invitado</v-btn>
            </template>
            <v-card>
              <v-card-title>
                <span class="headline">Registro de invitados</span>
              </v-card-title>
              <v-form ref="form" v-model="valid">
                        ...
                        >
                          <template v-slot:activator="{ on }">
                            <v-text-field
                              ...
                              prepend-icon="event"
                             @blur="editedVisit.
                             fecha = parseDate(dateFormatted)"
                              v-on="on"
                            ></v-text-field>
                          </template>
                          <v-date-picker locale="es" v-model="editedVisit.fecha" no-title @input="menu1 = false"></v-date-picker>
                        </v-menu>
                      </v-col>
                ...
                <v-card-actions>
                  ...
                  <v-btn
                    ...
                    @click.prevent="addPostulante(), dialog= false, getVisitas()"
                  >Registrar</v-btn>
                </v-card-actions>
              </v-form>
            </v-card>
...
export default {
  ...
  data(vm) {
    return {
      ...
      dniRules: [
        v => !!v || "El número de numDoc es requerido",
  ...
  computed: {
     computedDateFormatted () {
        return this.formatDate(this.editedVisit.fecha)
      },
  },
  ...
  watch: {
    date (val) {
        this.dateFormatted = this.formatDate(this.editedVisit.fecha)
      },
  },
